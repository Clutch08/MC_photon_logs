...
try:
    ...
    GITHUB_TOKEN = None
    _GITHUB_AVAILABLE = False
    print("[Warning] github_credentials.py not found. Upload disabled for this session.")
...
OUTPUT_FILE = Path.home() / "Desktop" / "Python" / "data_logs" / f"{SAMPLE_NAME}_scan.csv"

# --- GitHub upload ---
SESSION_NUMBER   = 1                      # increment manually each session
GITHUB_REPO = "Clutch08/MC_photon_logs"
GITHUB_BRANCH    = "main"
GITHUB_DATA_ROOT = "data"                 # top-level folder inside the repo
...
class KDC101Stage:
    ...
    def stop_motor(self):
        """
        Send MGMSG_MOT_MOVE_STOP to the KDC101 with profiled deceleration.
        Call this on KeyboardInterrupt to halt any in-progress move cleanly.
        MGMSG_MOT_MOVE_STOP = 0x0465
        Param2 = 0x02 — profiled stop (controlled deceleration).
        """
        self.ser.write(bytes([0x65, 0x04, 0x01, 0x02, 0x50, 0x01]))
...
def upload_to_github(local_path, token, repo_name, branch,
                     data_root, session_number, wavelength_nm,
                     pmma_thickness, scan_date_str):
    try:
        g = Github(token)
        repo = g.get_repo(repo_name)

        folder_path = (
            f"{data_root}/session_{session_number:03d}_{wavelength_nm}nm_{scan_date_str}"
        )
        base_stem = f"{wavelength_nm}_{pmma_thickness}mm_pmma"

        candidate = base_stem + ".csv"
        counter = 2
        while True:
            try:
                repo.get_contents(f"{folder_path}/{candidate}")
                candidate = f"{base_stem}-{counter}.csv"
                counter += 1
        ...
        response = repo.create_file(
            path=f"{folder_path}/{final_filename}",
            message=f"Add scan: {final_filename} (session {session_number:03d})",
            content=content,
            branch=branch,
        )

        print(f"Uploaded to GitHub: {response['content'].html_url}")
...
def main():
    ...
    try:
        with open(OUTPUT_FILE, "w", newline="") as f:
            ...
            ax.set_title(f"Live scan — {WAVELENGTH_NM} nm  {PMMA_THICKNESS} mm PMMA")
            ax.set_xlim(START_DEG, STOP_DEG)
            ax.set_yscale("log")
            (line,) = ax.plot([], [], "o-", markersize=3, linewidth=1)
            fig.canvas.draw()
            plt.pause(0.01)
            ...
            for sweep in range(NUM_SWEEPS):
                ...
                for angle in angles:
                    ...
                    angles_collected.append(actual_angle)
                    means_collected.append(mean_W)
                    line.set_xdata(angles_collected)
                    line.set_ydata(means_collected)
                    ax.relim()
                    ax.autoscale_view()
        ...
        print("\n[Interrupted] Stopping motor. Saving data collected so far...")
        stage.stop_motor()
    ...
    if _GITHUB_AVAILABLE:
        upload_to_github(
            ...
            data_root       = GITHUB_DATA_ROOT,
            session_number  = SESSION_NUMBER,
            wavelength_nm   = WAVELENGTH_NM,
            pmma_thickness  = PMMA_THICKNESS,
